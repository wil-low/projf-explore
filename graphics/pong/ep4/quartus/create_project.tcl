...
if {[is_project_open]} {
	if {[string compare $quartus(project) "pong"]} {
		puts "Project pong is not open"
		set make_assignments 0
	}
} else {
	# Only open if not already open
	if {[project_exists pong]} {
		project_open -revision top pong
	} else {
		project_new -revision top pong
	}
...
if {$make_assignments} {
	...
	set_global_assignment -name TOP_LEVEL_ENTITY top_pong
	...
	set_global_assignment -name SYSTEMVERILOG_FILE ../../../../lib/clock/ep4/clock_480p.sv
	...
	set_global_assignment -name SYSTEMVERILOG_FILE ../top_pong.sv
	set_global_assignment -name QIP_FILE ../../../../lib/clock/ep4/pll_25m.qip
